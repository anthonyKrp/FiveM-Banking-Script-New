...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' gestort', type = 'primary', lenght = '2500'})
		sendtoDiscord("**Spelernaam:** " .. GetPlayerName(source) .. "\n**Type:** Storten \n **Bedrag:** " .. amount .. " Euro")
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt niet genoeg geld op je', type = 'error', lenght = '2500'})
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' opgenomen', type = 'primary', lenght = '2500'})
		sendtoDiscord("**Spelernaam:** " .. GetPlayerName(source) .. "\n**Type:** Opnemen \n **Bedrag:** " .. amount .. " Euro")
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt niet genoeg geld op de bank', type = 'error', lenght = '2500'})
	end
end)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
						...
						TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' ontvangen van '..xPlayer.getName(), type = 'primary', lenght = '2500'})
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, source)
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' overgemaakt naar '..xTarget.getName(), type = 'primary', lenght = '2500'})
				sendtoDiscord("**Zender:** " .. GetPlayerName(source) .. "\n**Ontvanger:** ".. targetName .." \n **Bedrag:** " .. amount .. " Euro")
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' overgemaakt naar '..targetName, type = 'primary', lenght = '2500'})
				sendtoDiscord("**Zender:** " .. GetPlayerName(source) .. "\n**Ontvanger:** ".. targetName .." \n **Bedrag:** " .. amount .. " Euro")
		...
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je kan niet geld naar jezelf overmaken', type = 'error', lenght = '2500'})
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' overgemaakt naar '..societyName, type = 'primary', lenght = '2500'})
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt niet genoeg geld op je', type = 'error', lenght = '2500'})
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('rocket-notify:Notify', source, { text = 'Er is iemand al aan het overmaken', type = 'error', lenght = '2500'})
		else
			...
			TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' ontvangen van '..societyName, type = 'primary', lenght = '2500'})
		end
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = "Je bedrijf heeft niet genoeg geld op de bank", type = 'error', lenght = '2500'})
	end
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt €'..amount..' overgemaakt naar '..societyNameTarget, type = 'primary', lenght = '2500'})
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je bedrijf heeft niet zoveel geld op de bank', type = 'error', lenght = '2500'})
	end
end)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Iban is aangepast naar: '..iban, type = 'primary', lenght = '2500'})
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt niet genoeg geld om je iban aantepassen. Nodig: €'..amount, type = 'primary', lenght = '2500'})
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', _source)
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Pincode aangepast naar '..pin, type = 'primary', lenght = '2500'})
	else
		TriggerClientEvent('rocket-notify:Notify', source, { text = 'Je hebt niet genoeg geld om je pin aantepassen. Nodig: €'..amount, type = 'primary', lenght = '2500'})
	end
end)

function sendtoDiscord(message)
    local WebHook = "https://discord.com/api/webhooks/882902671881797632/_lSCfR8qp8k6TYh4-y-Z3jrJW3xLtBNXv3WxfVxAjZ06D5AZ0rLnCp41X_-wQjcoS14W"

    local discordInfo = {
        ["color"] = "3447003",
        ["type"] = "rich",
        ["title"] = "[Atlantis Logging]",
        ["description"] = message,
        ["footer"] = {
        ["text"] = "Atlantis Banking Loggs"
        }
    }
    PerformHttpRequest(WebHook, function(err, text, headers) end, 'POST', json.encode({ username = 'Atlantis', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end