...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(Config.BankLocations) do
			...
			if dist < 2 then
				inRange = true
				doDrawText(vector3(v.x, v.y, v.z), '~b~E~w~ - Bank')
				...
				if dist < 6 then
					inRange = true
					doDrawText(vector3(v.x, v.y, v.z), 'Bank')
				end
			end
		end
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("Druk ~INPUT_PICKUP~ om de ~b~pinautomaat~w~ te openen")
            else
            	inRange = true
            end
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						exports['rocket-notify']:Notify("Ga naar de bank om een pincode in te stellen", 'primary', '3000')
					end
				end)
            end
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['rocket-notify']:Notify("Ongeldige Hoeveelheid", 'error', '3000')
			end
		else
			exports['rocket-notify']:Notify("Ongeldige invoer", 'error', '3000')
		end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						exports['rocket-notify']:Notify("Dit iban bestaat niet", 'error', '3000')
					end
				end, data.iban:upper())
			else
				exports['rocket-notify']:Notify("Ongeldige Hoeveelheid", 'error', '3000')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					exports['rocket-notify']:Notify("Dit iban is al in gebruik", 'error', '3000')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['rocket-notify']:Notify("Je kan alleen nummers gebruiken in je iban", 'error', '3000')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				exports['rocket-notify']:Notify("Je pincode moet 4 cijfers zijn", 'error', '3000')
			end
		else
			exports['rocket-notify']:Notify("Je kan alleen nummers gebruiken", 'error', '3000')
		end
	end
end)
